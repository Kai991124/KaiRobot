...
def search_jx3(command):
    ...
    if (map_search):
        ...
        if (map_requests['msg'] == 'success'):
            ...
            for i in range(len(data)):
                send_msg('家具名：{0}'.format(data[i]['name']))
                send_msg('家具属性：\n-观赏分：{0}，实用分：{1}，坚固分：{2}，风水分：{3}，趣味分：{4}'.format(data[i]['view_score'],
                                                                               data[i]['practical_score'],
                                                                               data[i]['hard_score'],
                                                                               data[i]['geomantic_score'],
                                                                               data[i]['interesting_score']))
        else:
            send_msg('你看看你输入的啥，我可不认识这个地图！')
    # 小药
    ...
    if (medicine_search):
        ...
        if (medicine_requests['msg'] == 'success'):
            ...
            send_msg('推荐小药-{0}，{1}，{2}，{3}'.format(data['heighten_food'],
                                                                           data['auxiliary_food'],
                                                                           data['heighten_drug'],
                                                                           data['auxiliary_drug'])                                                                          )
        else:
...
def dingqun():
    ...
    if (sort>=0 and sort<=60):
        send_msg(requests.get('https://www.jx3api.com/app/random').json()['data']['text'])
